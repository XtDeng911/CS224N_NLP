...
class SoftmaxModel(Model):
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None):
        ...
        """
        if type(labels_batch)!=type(None):
    ...
    def add_prediction_op(self):
        ...
        """
        self.W=tf.Variable(np.zeros((self.config.n_features,self.config.n_classes)),dtype=tf.float32)
        self.b=tf.Variable(np.zeros(self.config.n_classes),dtype=tf.float32)
        pred=softmax(tf.matmul(self.input_placeholder,self.W)+tf.expand_dims(self.b,0))
